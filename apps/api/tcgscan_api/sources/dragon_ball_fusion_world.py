"""Dragon Ball Super Card Game Fusion World — Bandai official card database."""
...
DEFAULT_BASE_URL = "https://www.dbs-cardgame.com/fw/en/cardlist/"
...
def _base_url() -> str:
    return os.getenv("DRAGON_BALL_FW_BASE_URL", DEFAULT_BASE_URL).strip()


def normalize_card(raw: dict[str, Any]) -> dict[str, Any]:
    return {
        "game": "dragon_ball_fusion_world",
        "source": "bandai",
...
class DragonBallFusionWorldClient:
        ...
        try:
            ...
            if found:
                return {
                    ...
                    "message": "Bandai Fusion World JSON endpoint reachable",
                }
            ...
            if status_code >= 400:
                return {
                    ...
                    "message": f"Bandai Fusion World page unreachable (HTTP {status_code})",
                }
