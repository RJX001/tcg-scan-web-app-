    ...
    async (offset: number, append: boolean) => {
      ...
      try {
        const out = await getSalesBrowse({
          ...
          soldAfter: toIsoDate(minDate, false),
          soldBefore: toIsoDate(maxDate, true),
          sort,
          limit: PAGE_SIZE,
          offset,
        });
    ...
    <div className="flex flex-col gap-4">
      <div className="flex gap-2">
        <input
          ...
          placeholder="Search sold comps by card, set, or number…"
      ...
      {showFilters && (
        <div className="grid gap-2 rounded-xl border border-zinc-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <input
            ...
            className="rounded-lg border border-zinc-300 px-3 py-2 text-sm"
            aria-label="Sold before"
          />
          <select value={game} onChange={(e) => setGame(e.target.value)} className="rounded-lg border border-zinc-300 px-3 py-2 text-sm">
            {GAMES.map((g) => (
              <option key={g.value} value={g.value}>{g.label}</option>
            ))}
          </select>
          <select value={source} onChange={(e) => setSource(e.target.value)} className="rounded-lg border border-zinc-300 px-3 py-2 text-sm">
            {PLATFORMS.map((p) => (
              <option key={p.value} value={p.value}>{p.label}</option>
            ))}
          </select>
          <select value={grade} onChange={(e) => setGrade(e.target.value)} className="rounded-lg border border-zinc-300 px-3 py-2 text-sm">
            {GRADES.map((g) => (
              <option key={g.value} value={g.value}>{g.label}</option>
            ))}
          </select>
          <Button variant="outline" onClick={() => { setGame(""); setSource(""); setGrade(""); setMinPrice(""); setMaxPrice(""); setMinDate(""); setMaxDate(""); }}>
            Clear
          </Button>
        </div>
      )}
        ...
        {SORTS.map((s) => (
          <button
            ...
            className={`rounded-full border px-3 py-1 text-xs font-medium ${
              sort === s.value ? "border-blue-700 bg-blue-700 text-white" : "border-zinc-300 bg-white text-zinc-600"
            }`}
          >
            {s.label}
          </button>
      ...
      <ul className="divide-y divide-zinc-100 overflow-hidden rounded-xl border border-zinc-200 bg-white">
        {rows.map((s, i) => {
          const img = thumb(s);
          return (
            <li key={`${s.sold_at}-${s.card.id}-${i}`} className="flex items-center gap-3 px-4 py-3">
              {img ? (
                <Image src={img} alt={s.card.name} width={40} height={56} className="shrink-0 rounded border border-zinc-200 object-cover" />
              ) : (
                <div className="h-14 w-10 shrink-0 rounded border border-zinc-200 bg-zinc-100" />
              )}
              <div className="min-w-0 flex-1">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-zinc-400">{s.source} · {s.market_region}</p>
                <Link href={`/card/${s.card.slug}`} className="block truncate font-medium hover:text-blue-700">
                  {s.card.name}
                  {s.card.number ? <span className="text-zinc-400"> #{s.card.number}</span> : null}
                </Link>
                <div className="mt-0.5 flex items-center gap-2">
                  <GradeBadge grade={s.grade} />
                  <span className="text-xs text-zinc-500">{fmtSoldAtLocal(s.sold_at)}</span>
                </div>
              </div>
              <div className="shrink-0 text-right">
                <p className="text-[11px] uppercase tracking-wide text-zinc-400">Sold</p>
                <p className="font-semibold">{fmtNative(s.price, s.currency)}</p>
                {s.listing_url && (
                  <a href={s.listing_url} target="_blank" rel="noopener noreferrer" className="text-xs font-medium text-blue-700 hover:underline">
                    View →
                  </a>
                )}
              </div>
      ...
      <div className="flex justify-center">
        {loading ? (
          ...
          <Button variant="outline" onClick={() => void load(rows.length, true)}>Load more</Button>
        ) : null}
      </div>
    </div>
  );
